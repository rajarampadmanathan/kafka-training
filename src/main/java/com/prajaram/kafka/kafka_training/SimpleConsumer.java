...
public class SimpleConsumer {
	...
	public static void consume(String[] args, int i, String groupName) throws InterruptedException, ExecutionException {
        ...
        //consumer.subscribe(Collections.singletonList("global-event"));
		consumer.subscribe(Collections.singletonList("test"));
        while(true) {
        	consumer.poll(10).forEach(record->{
        		System.out.println("Group:"+groupName+"&& Consumer"+i+":"+record.key()+record.value()+record.headers()+"-Partition:"+record.partition());
